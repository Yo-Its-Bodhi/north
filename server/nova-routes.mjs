...
function validateProposal(input){
  ...
  if(actionType==="create_workout"){if(!Array.isArray(payload.exerciseIds))return null;payload.exerciseIds=[...new Set(payload.exerciseIds.map(String).filter((id)=>exerciseCoachingIds.has(id)))].slice(0,12);if(!payload.exerciseIds.length)return null;payload.saveToMyWorkouts=payload.saveToMyWorkouts===true;}
