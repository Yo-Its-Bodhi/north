		...
		{sets.map((set, index) => {
			...
			return <div key={index} className={`set-row dynamic-set-row ${set.complete ? "set-complete" : ""} ${missingCount > 0 ? "set-has-missing" : ""}`} style={{ gridTemplateColumns }}>
				...
				{fields.map((field) => {
					...
					return <label key={field.id} className={`dynamic-set-field ${isMissing ? "field-missing" : ""}`}><input type="number" inputMode="decimal" min={field.minimum} max={field.maximum} value={String(value)} onChange={(event) => updateValue(index, field, event.target.value)} aria-label={`Set ${index + 1} ${field.label}`} /><small>{fieldUnit(field, preferences).replaceAll("_", "/")}</small></label>;
				})}
