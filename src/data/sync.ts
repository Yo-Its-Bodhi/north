...
export async function syncNorth(apiBase: string, accessToken: string): Promise<SyncResult> {
  const mutations = (await northRepository.pendingMutations()).filter((mutation) => new Date(mutation.nextAttemptAt).getTime() <= Date.now());
  let sent = 0;
  let conflicts = 0;
  let failed = 0;
...
export async function pullNorth(apiBase: string, accessToken: string, since = "1970-01-01T00:00:00.000Z"): Promise<PullResult> {
  ...
  return { restored: result.documents.length, serverTime: result.serverTime };
}
