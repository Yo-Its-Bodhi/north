...
})).map((exercise) => exercise.name === "EZ-bar curl 21s" || exercise.name === "Dumbbell curl 21s" ? {
  ...exercise,
  target: "3 sets · 7 lower + 7 full + 7 upper reps",
  rest: 75,
  cue: "Complete seven lower-half curls, seven full curls, then seven upper-half curls without swinging the torso.",
} : exercise);
