...
export default function Onboarding({ onComplete, onLocalPreview }: Props) {
  ...
  if (!session && step === 0) return <main className="onboarding-shell"><section className="onboarding-card auth-card">
    {mode === "welcome" ? <><div className="welcome-brand"><div className="welcome-lockup" aria-label="North"><img className="welcome-lockup-light" src="/png/transparent/footprint-clean-teal.png" alt="North" /><img className="welcome-lockup-dark" src="/png/transparent/footprint-clean-offwhite.png" alt="North" /></div><p className="welcome-tagline"><span>Find your</span> <span>direction.</span></p></div><div className="welcome-content"><p className="welcome-heading">Welcome</p><p className="onboarding-kicker">YOUR PLAN. YOUR PROGRESS. YOUR NORTH.</p><p className="onboarding-lead">North brings your training, recovery and progress into one calm place, with guidance grounded in your own record.</p><div className="welcome-points"><div><Check />Build and adapt a plan around real life</div><div><Check />Track training, recovery and meaningful progress</div><div><Check />Work with Nova—review every suggested change</div></div></div><div className="welcome-actions"><button className="onboarding-primary" onClick={() => setMode("register")}>Create your North <ArrowRight /></button><button className="onboarding-secondary" onClick={() => setMode("login")}>I already have an account</button>{onLocalPreview && <button className="onboarding-preview" onClick={onLocalPreview}>Preview locally</button>}</div></> : <>
