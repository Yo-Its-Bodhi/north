...
import { ArrowLeft, ArrowRight, Check, CircleHelp, Compass, Copy, Eye, EyeOff, KeyRound, ShieldCheck, Sparkles } from "lucide-react";
import { loginNorthAccount, NORTH_API_BASE, NORTH_LEGAL_NOTICE_VERSION, readNorthSession, recoverNorthAccount, registerNorthAccount, type NorthSession } from "./data/account";
import { pullNorth } from "./data/sync";
import LegalNotice from "./components/LegalNotice";

export type OnboardingResult = {
  name: string; direction: string; trainingDays: number; weightUnit: "lb" | "kg"; distanceUnit: "mi" | "km";
...
function readDraft() {
  try { return JSON.parse(localStorage.getItem(ONBOARDING_KEY) || "{}") as Partial<OnboardingResult> & { units?: "imperial" | "metric" }; } catch { return {}; }
}
...
export default function Onboarding({ onComplete, onLocalPreview }: Props) {
  ...
  const [weightUnit, setWeightUnit] = useState<"lb" | "kg">(draft.weightUnit ?? (draft.units === "metric" ? "kg" : "lb"));
  const [distanceUnit, setDistanceUnit] = useState<"mi" | "km">(draft.distanceUnit ?? (draft.units === "metric" ? "km" : "mi"));
  ...
  const legalNoticeButtonRef = useRef<HTMLButtonElement>(null);
  ...
  const saveDraft = () => localStorage.setItem(ONBOARDING_KEY, JSON.stringify({ name, direction, experience, trainingDays, trainingDayIndexes, duration, equipment, activities, weightUnit, distanceUnit, memoryEnabled }));
  ...
  async function authenticate(kind: "register" | "login" | "recover") {
    if (kind === "register" && !legalAccepted) { setError("You must read and agree to the Legal & Safety Notice before creating an account."); return; }
    setBusy(true); setError("");
    try {
      const next = kind === "register" ? await registerNorthAccount(username, password, name, accessCode, NORTH_LEGAL_NOTICE_VERSION) : kind === "login" ? await loginNorthAccount(username, password) : await recoverNorthAccount(username, recoveryCode, password);
  ...
  function finish() {
    const result = { name, direction, experience, trainingDays, trainingDayIndexes, duration, equipment, activities, weightUnit, distanceUnit, memoryEnabled };
    localStorage.setItem(`north-onboarding-complete:${session?.user.id}`, new Date().toISOString());
    localStorage.removeItem(ONBOARDING_KEY);
    onComplete(result);
  }

  if (!session && step === 0 && legalNoticeOpen) return <main className="legal-entry-shell"><LegalNotice onBack={() => { setLegalNoticeOpen(false); requestAnimationFrame(() => legalNoticeButtonRef.current?.focus()); }}/></main>;

  if (!session && step === 0) return <main className="onboarding-shell"><section className="onboarding-card auth-card">
    {mode === "welcome" ? <><div className="welcome-brand"><div className="welcome-lockup" aria-label="North"><img className="welcome-lockup-light" src="/png/transparent/footprint-clean-teal.png" alt="North" /><img className="welcome-lockup-dark" src="/png/transparent/footprint-clean-offwhite.png" alt="North" /></div><p className="welcome-tagline"><span>Find your</span> <span>direction.</span></p></div><div className="welcome-content"><p className="welcome-heading">Your training has a home.</p><p className="onboarding-kicker">PLAN IT. TRAIN IT. UNDERSTAND IT.</p><p className="onboarding-lead">Build your week, train with a serious workout system, connect your health data and see the patterns in your Training Atlas. Nova is there whenever you want to talk it through.</p><div className="welcome-points"><div><Check />Build workouts, follow programs and adapt the plan to real life</div><div><Check />Keep training, health, milestones and progress in one private record</div><div><Check />Explore Community workouts, North Originals and 785 reviewed exercises</div></div></div><div className="welcome-actions"><button className="onboarding-primary" onClick={() => setMode("register")}>Create your North <ArrowRight /></button><button className="onboarding-secondary" onClick={() => setMode("login")}>I already have an account</button>{onLocalPreview && <button className="onboarding-preview" onClick={onLocalPreview}>Preview North</button>}</div></> : <>
      ...
      <div className="onboarding-field"><div className="password-label-row"><label htmlFor="north-password"><span>{mode === "recover" ? "New password" : "Password"}</span></label><span className="password-requirements"><button className="password-requirements-trigger" type="button" aria-label="Password requirements" aria-expanded={passwordRequirementsOpen} aria-controls="password-requirements-popover" onClick={() => setPasswordRequirementsOpen((open) => !open)} onKeyDown={(event) => { if (event.key === "Escape") setPasswordRequirementsOpen(false); }}><CircleHelp /></button>{passwordRequirementsOpen && <span id="password-requirements-popover" className="password-requirements-popover" role="tooltip"><strong>Password requirements</strong><span>Use at least 10 characters.</span><small>No uppercase letter, number, or symbol is required.</small></span>}</span></div><span className="password-field"><input id="north-password" type={passwordVisible ? "text" : "password"} autoComplete={mode === "login" ? "current-password" : "new-password"} value={password} onChange={(event) => setPassword(event.target.value)} placeholder="At least 10 characters" /><button type="button" aria-label={passwordVisible ? "Hide password" : "Show password"} aria-pressed={passwordVisible} onClick={() => setPasswordVisible((visible) => !visible)}>{passwordVisible ? <EyeOff /> : <Eye />}</button></span></div>
      {mode === "register" && <div className="legal-consent"><label><input type="checkbox" checked={legalAccepted} onChange={(event) => setLegalAccepted(event.target.checked)} /><span><strong>I agree to North's Legal &amp; Safety Notice.</strong><small>I understand that North and Nova are fitness education, planning and workout-tracking tools, not medical care or professional advice; automated guidance may be inaccurate; exercise carries risk; and I remain responsible for my training decisions.</small></span></label><button ref={legalNoticeButtonRef} type="button" onClick={() => setLegalNoticeOpen(true)}>Read the full Legal &amp; Safety Notice</button></div>}
      {error && <p className="onboarding-error" role="alert">{error}</p>}
      <button className="onboarding-primary" disabled={busy || username.length < 3 || password.length < 10 || (mode === "register" && (!name.trim() || !legalAccepted))} onClick={() => authenticate(mode === "recover" ? "recover" : mode)}>{busy ? "Working…" : mode === "register" ? "Create account" : mode === "login" ? "Sign in" : "Recover account"}<ArrowRight /></button>
  ...
  return <main className="onboarding-shell"><section className="onboarding-card setup-card">
    <header><div className="onboarding-brand"><img className="onboarding-brand-lockup" src="/png/transparent/lockup-horizontal-teal.png" alt="North" /></div><small>{step === 1 ? "SECURE YOUR ACCOUNT" : `SETUP ${Math.max(1, step - 1)} OF 5`}</small></header>
    ...
    {step === 5 && <div><p className="onboarding-kicker">HOW NORTH MEETS YOU</p><h1>Keep it comfortable and yours.</h1><label><span>Weight</span><select value={weightUnit} onChange={(event) => setWeightUnit(event.target.value as "lb" | "kg")}><option value="lb">Pounds (lb)</option><option value="kg">Kilograms (kg)</option></select></label><label><span>Distance</span><select value={distanceUnit} onChange={(event) => setDistanceUnit(event.target.value as "mi" | "km")}><option value="mi">Miles (mi)</option><option value="km">Kilometres (km)</option></select></label><label className="memory-choice"><input type="checkbox" checked={memoryEnabled} onChange={(event) => setMemoryEnabled(event.target.checked)} /><span><strong>Remember helpful patterns</strong><small>Let North notice useful things across your records. You can review or forget them anytime.</small></span></label></div>}
