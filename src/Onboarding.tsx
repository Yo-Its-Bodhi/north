...
import { ArrowLeft, ArrowRight, Check, Compass, Copy, Eye, EyeOff, KeyRound, ShieldCheck, Sparkles } from "lucide-react";
...
export default function Onboarding({ onComplete, onLocalPreview }: Props) {
  ...
  if (!session && step === 0) return <main className="onboarding-shell"><section className="onboarding-card auth-card">
    {mode === "welcome" ? <><div className="welcome-brand"><div className="welcome-lockup" aria-label="North"><img className="welcome-lockup-light" src="/png/transparent/footprint-clean-teal.png" alt="North" /><img className="welcome-lockup-dark" src="/png/transparent/footprint-clean-offwhite.png" alt="North" /></div><p className="welcome-tagline"><span>Find your</span> <span>direction.</span></p></div><div className="welcome-content"><p className="welcome-heading">Your training has a home.</p><p className="onboarding-kicker">PLAN IT. TRAIN IT. UNDERSTAND IT.</p><p className="onboarding-lead">Build your week, train with a serious workout system, connect your health data and see the patterns in your Training Atlas—with Nova there to help you think, never take over.</p><div className="welcome-points"><div><Check />Build workouts, follow programs and adapt the plan to real life</div><div><Check />Keep training, health, milestones and progress in one private record</div><div><Check />Explore Community workouts, North Originals and 784 reviewed exercises</div></div></div><div className="welcome-actions"><button className="onboarding-primary" onClick={() => setMode("register")}>Create your North <ArrowRight /></button><button className="onboarding-secondary" onClick={() => setMode("login")}>I already have an account</button>{onLocalPreview && <button className="onboarding-preview" onClick={onLocalPreview}>Preview North</button>}</div></> : <>
      ...
      <div className="onboarding-field"><label htmlFor="north-password"><span>{mode === "recover" ? "New password" : "Password"}</span></label><span className="password-field"><input id="north-password" type={passwordVisible ? "text" : "password"} autoComplete={mode === "login" ? "current-password" : "new-password"} value={password} onChange={(event) => setPassword(event.target.value)} placeholder="At least 10 characters" /><button type="button" aria-label={passwordVisible ? "Hide password" : "Show password"} aria-pressed={passwordVisible} onClick={() => setPasswordVisible((visible) => !visible)}>{passwordVisible ? <EyeOff /> : <Eye />}</button></span></div>
