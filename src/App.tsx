...
function App() {
  ...
  async function applyApiProposal(message: NovaMessage) {
    ...
    try {
      ...
      setNovaMessages((items) => items.map((item) => item.id === message.id ? { ...item, apiProposal: { ...item.apiProposal!, status: "applied" }, appliedAt: new Date().toISOString() } : item));
      ...
      {screen === "nova" && (
        <section className="screen destination-screen nova-screen">
          ...
          <section className="conversation-surface" ref={novaTranscriptRef}>
            ...
            {novaMessages.filter((message) => message.apiProposal).map((message) => <section className={`nova-proposal api-proposal ${message.apiProposal!.status}`} key={`proposal-${message.id}`}><header><span>{message.apiProposal!.status === "applied" ? <Check size={15}/> : <Sparkles size={15}/>}</span><div><small>{message.apiProposal!.status === "applied" ? "CHANGE APPLIED" : "NOVA PROPOSAL · YOUR APPROVAL REQUIRED"}</small><strong>{message.apiProposal!.summary}</strong></div></header><p>{message.apiProposal!.reason}</p><div className="nova-proposal-payload"><span>{message.apiProposal!.action_type.replaceAll("_", " ")}</span><small>Nova cannot apply this without your confirmation.</small></div>{message.apiProposal!.status === "pending" && <div className="nova-proposal-actions"><button onClick={() => void rejectApiProposal(message)}>Not now</button><button onClick={() => void applyApiProposal(message)}>Review & confirm</button></div>}{message.apiProposal!.status === "applied" && <div className="nova-action-receipt"><Check size={14}/><span>Saved to your North account. It will sync with your other devices.</span></div>}{message.apiProposal!.status === "rejected" && <div className="nova-action-receipt"><X size={14}/><span>No change was made.</span></div>}</section>)}
