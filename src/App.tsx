    ...
    <main className="app-shell" id="north-content" tabIndex={-1}>
      ...
      <header className="topbar">
        ...
        <div className="topbar-actions">
          ...
          {readNorthSession() && <button className={`topbar-account-button ${screen === "account" ? "active" : ""}`} onClick={() => setScreen("account")} aria-label="Open account and settings"><UserRound size={18} /><span>{(profile.name || readNorthSession()?.user.displayName || "N").slice(0,1).toUpperCase()}</span></button>}
        </div>
      </header>
